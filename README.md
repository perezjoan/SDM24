...
_This project is the result of a workshop titled "SDM Statistics and Data Mining," organized by UCA Côte d'Azur University as part of the Master’s program in Geoprospective Aménagement et Durabilité des Territoires (GeoPRAD). The workshop took place over two and a half days and involved seven Master 2-level students, who were divided into three groups of two people, with one project lead. The objective of the workshop was to develop a fully working geospatial analysis protocol by the end of the session. The GitHub repository contains the results of this collaborative effort.It is important to note that, due to the time constraints of the workshop, there are some known issues with the code. Module 2 does not use a fixed random seed, which may result in variability in the clustering results. Module 3 is only functional for a clustering solution involving seven clusters. Additionally, the line plot in Module 3 is not displayed correctly, and one page in the PDF report generated by ReportLab does not render properly.Despite these issues, the code remains functional and can be used by anyone interested in applying or extending the analysis for their own purposes. Users are encouraged to adapt the code and address any issues as needed._
